...
from adventureworks_customer c join sales s 
on c.customerkey=s.customerkey join products p 
on s.productkey=p.productkey 
group by c.gender
order by revenue desc;
/*==================================================REVENUE BY OCCUPATION===================================================================================*/
select 
distinct c.occupation,
round(sum(s.orderquantity*p.productprice),2) as revenue 
from adventureworks_customer c join sales s 
on s.customerkey=c.customerkey join products p 
on p.productkey=s.productkey 
...
==================================================================================================================================================*/
select 
c.educationlevel,(sum(s.orderquantity*p.productprice)/sum(s.orderquantity)) as aov,sum(s.orderquantity) as ordercount,
round(sum(s.orderquantity*p.productprice),2) as revenue
 from products p join sales s 
 on p.productkey=s.productkey 
 join adventureworks_customer c 
 on s.customerkey=c.customerkey 
 group by c.educationlevel
 order by revenue desc;
/*=================================================================================================================================================
                                        DO RICH CUSTOMERS SPENDS THE MOST?
====================================================================================================================================================*/
with income_cluster as (select customerkey,ntile(5) over(order by annualincome desc) as income_clusters from adventureworks_customer)
select 
c.income_clusters,count(distinct c.customerkey),sum(s.orderquantity*p.productprice)/count(distinct c.customerkey) as aov,
round(sum(s.orderquantity*p.productprice),2) as revenue 
from income_cluster c join sales s
 on s.customerkey=c.customerkey join products p 
 on p.productkey=s.productkey 
 group by c.income_clusters 
 order by revenue desc;
